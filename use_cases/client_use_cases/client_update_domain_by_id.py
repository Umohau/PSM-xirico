...
class ClientUpdateDomainById:
    ...
    def execute(self, dados: ClientUpdateDTO) -> Result[UpdateOutputDTO, BaseDomainError| ClientError]:
        logger.debug(
            'iniciando processo de actualozacao de dominio do cliente'
        )
        new_domain: dict={'dominio': dados.domain}
        warnings: list[BaseDomainError]= list()
        try:
            logger.debug(
                'recuperando o dominio actual'
            )
            old_domain: str= self._repo.search_id(dados.client_id).get('dominio')
            logger.debug(
                'actualizando dominio do cliente %s', dados.client_id
            )
            effect= self._repo.update(dados_= new_domain, id= dados.client_id)
            logger.info(
                 'dominio do cliente %s actualizado com sucesso', dados.client_id
            )
        ...
        try:
            logger.debug('auditando a accao de actualizacao de dominio do cliente')
            self.audit.auditar(
                operador= self._profile.id,
                operacao= 'client_update_domain_by_id',
                detalhes= f'actualizou o dominio do cliente com id {dados.client_id}'
            )
            logger.debug(
            ...
            logger.warning(
                'falha no registro de auditoria da operacao de actualizacao de dominio do cliente',
                exc_info=True
            )

...
class ClientUpdateEmail:
    ...
    def execute(self, dados: ClientUpdateDTO) -> Result[UpdateOutputDTO, BaseDomainError| ClientError]:
        logger.debug(
            'iniciando operacao de actualizacao de email do cliente'
        )
        ...
        try:
            ...
            logger.debug(
                'actualizando email do cliente %s', dados.client_id
            )
            effect= self._repo.update(dados_= new_email, id= dados.client_id)
            logger.info(
                'email do cliente %s actualizado com sucesso', dados.client_id
            )
        ...
        try:
            logger.debug('auditando a accao de actualizacao de email do cliente')
            self.audit.auditar(
                operador= self._profile.id,
                operacao= 'client_update_email',
                detalhes= f'actualizou o email do cliente com id {dados.client_id}'
            )
            logger.debug(
            ...
            logger.warning(
                'falha no registro de auditoria da operacao de actualizacao de email do cliente',
                exc_info=True
            )

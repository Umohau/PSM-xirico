...
@pytest.fixture
def update_bird_name_in_catalog(mock_repo, mock_audit, mock_profile):
    return UpdateBirdNameInCatalogById(
        repo= mock_repo,
...
def test_update_bird_name_in_catalog_by_id_sucess(
        mock_repo,
        mock_audit,
        update_bird_name_in_catalog
):
    mock_repo.search_id.return_value= {'nome_comum': 'nowan'}
    mock_repo.update.return_value= 1
    res= update_bird_name_in_catalog.execute(dados= dados)
    output= res.unwrap()
...
def test_update_bird_name_bird_not_found(
        mock_repo,
        mock_audit,
        update_bird_name_in_catalog
):
...
def test_update_bird_name_audit_failled(
        mock_audit,
        mock_repo,
        update_bird_name_in_catalog
):
    ...
    res= update_bird_name_in_catalog.execute(dados)
    output= res.unwrap()
    mock_repo.update.assert_called()
    mock_audit.auditar.assert_called()
    assert is_ok(res)
...
def test_update_bird_name_in_catalog_conexion_error(
        mock_repo,
        mock_audit,
        update_bird_name_in_catalog
):
